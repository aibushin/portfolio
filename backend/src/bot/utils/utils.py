...
from src.bot.keyboards.kbs import main_keyboard
...
✨ Наша миссия:
Мы стремимся раскрыть вашу уникальную красоту,
подчеркнуть индивидуальность и подарить уверенность в себе.
...
👩‍🎨 Наши мастера:
Талантливая команда профессионалов с многолетним опытом и постоянным стремлением к совершенству.
Мы следим за последними трендами и используем инновационные техники.

🌿 Наша атмосфера:
Погрузитесь в атмосферу уюта и релаксации. Каждый визит к нам - это не просто процедура,
а настоящий ритуал красоты и заботы о себе.
...
@connection
async def save_receipt(data: dict, session: AsyncSession):
    schema = OFDReceiptSchema(**data["request"], **data["data"]["json"])
    validated_data = schema.model_dump()
    receipt = await OFDReceiptDAO.add_receipt_with_items(
        session=session, data=validated_data
    )
    return receipt.qrraw
